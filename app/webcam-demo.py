...
def make_grid(avg, frame, dilated, person_prob):
    ...
    with_scores = np.zeros((frame.shape[0], frame.shape[1], 3), np.uint8)
    cv2.putText(with_scores, 'Person Proba: {}'.format(person_prob), (0, 20),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)    

    grid_top = np.concatenate((frame, avg_rgb), axis=1)
    grid_bottom = np.concatenate((dilated_rgb, with_scores), axis=1)
...
while(True):
    ...
    grid = make_grid(avg, frame, dilated, person_prob)
