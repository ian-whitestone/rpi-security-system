...
class BaseCamera(object):
    ...
    frames = [] # list of frames & metadata for storage
...
class Camera(BaseCamera):
    ...
    def frames(self):
        ...
        with picamera.PiCamera() as camera:
            ...
            for f in camera.capture_continuous(raw_capture, 'bgr',
                ...
                top = np.concatenate((response['gray'], avg), axis=1)
    ...
    def store_frame(self, frame, occupied, timestamp):
        ...
        BaseCamera.frames.append(frame_dict)

        if len(BaseCamera.frames) > self.frame_store_cnt:
            BaseCamera.frames.pop(0)
        return
    ...
    def process_response(self, response):
        ...
        if occupied:
            ...
            elapsed = (timestamp - last_occ_uploaded).seconds
            ...
            BaseCamera.motion_counter = 0
            elapsed = (timestamp - last_unocc_uploaded).seconds
            if elapsed >= self.unocc_min_upload_seconds:
                last_unocc_uploaded = timestamp
                if self.save_images:
                    utils.save_image(filepath, frame)
        return
